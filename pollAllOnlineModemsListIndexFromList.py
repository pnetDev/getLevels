...
#CM 18/05/18 	Usage, the user supplies the CMTS and upstream interface ID of the CMTS, or it is read from a list.
#		The script will generate a list of modems found online on the CMTS upstream interface using cmtscmstatustable. A shell script is called to do this.
#		The levels and error counters, us and ds errors, are recorded. This runs twice.
#		The percentage of DS hec errors is calculated and the delta in US HEC errors is calculated.
#
#		This version reads the cmts name and upstream interface ID from a text file and is iterated.
...
for cmtsInfo in open("cmtsListALL",'r'):
	cmtsInfo = cmtsInfo.split(" ")
	cmts = cmtsInfo[0]
	usIF = cmtsInfo[1]
	print cmts,usIF
	...
	for data in range(modemCount):
		try:
			...
			print "An error occured assigning Stats variable"
			continue
		# Sanity print
