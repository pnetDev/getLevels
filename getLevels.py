...
def getOid(oid):
	...
	snmpData  = ( session.get(vars) )
	snmpData = snmpData.replace("(","") ; snmpData = snmpData.replace(")","") ; snmpData = snmpData.replace("'","") ; snmpData = snmpData.replace(",","") ## Clean up the string
	return  snmpData
...
MAC = getOid(mac)
mac = ':'.join('{:02x}'.format(ord(x)) for x in mac.value)## This nifty line gives the proper MAC address format.
hexMac = MAC
MAC = convert(MAC)
dsFreq = getOid(dfq) 	
usFreq = getOid(ufq) 	
...
MAC = getOid(mac)
myMAC = ':'.join('{:02x}'.format(ord(x)) for x in MAC.value)  ## This nifty line gives the proper MAC address format.
dsFreq = getOid(dfq)
usFreq = getOid(ufq)
DSP = getOid(dsp)
SNR = getOid(snr)
